...
import { CustomDarkTheme, CustomLightTheme, IThemeInterface } from '../theme/colors'
...
const ThemeProvider: FC<ThemeProps> = ({ children }) => {
  ...
  const getSavedTheme = async () => {
    ...
    if (themeMode !== null){
      themeMode === 'light' ? setTheme(CustomLightTheme) : setTheme(CustomDarkTheme);
    }
  ...
  const updateTheme = (currentThemeMode: "light" | "dark") => {
    const newTheme = currentThemeMode === 'light' ? CustomLightTheme : CustomLightTheme;
    setTheme(newTheme);
    AsyncStorage.setItem("theme", newTheme.mode ? "dark" : "light");
  };
...
export const useTheme = () => useContext(ThemeContext) as IThemeContextInterface;
